...
def engagement(target: str, session_path: str, depth: int, profile: str,
    ...
    # Token-auth SPA (bearer JWT in sessionStorage): the token is short-lived (EHRM ~30 min), so a
    # long scan must re-mint it or it silently 401s mid-run. Wire a refresh that re-logs-in via the
    # captured profile (creds from its *_env vars, e.g. EHRM_USERNAME/EHRM_PASSWORD) and returns the
